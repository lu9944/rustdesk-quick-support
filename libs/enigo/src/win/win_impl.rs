...
fn keybd_event(flags: u32, vk: u16, scan: u16) -> DWORD {
    ...
    unsafe {
        // KEYBDINPUT is smaller than the INPUT.u union (MOUSEINPUT-sized), so we
        // must not transmute_copy it (would read out of bounds). Write it into a
        // zeroed INPUT instead.
        let p = std::ptr::addr_of_mut!(input.u) as *mut KEYBDINPUT;
        std::ptr::write(
            p,
            KEYBDINPUT {
                wVk: vk,
